...
--          )  return Sys_GetSizeOf_Ptr;
   ...
   procedure Search (Major : Natural) is
      ...
      procedure Scan (Path : String; Descend : Natural := 0) is
         Items : Search_Type;
   ...
   begin
      if not Searched then
         Searched := True;
--           Scan ("/usr/lib", 2);
         Scan ("/Library/Frameworks/Python.framework/Versions", 3);
--           Scan ("/usr/local/Cellar", 8);
         Scan ("/sw/lib", 2);
         Scan ("/opt/lib", 2);
         Scan ("/opt/homebrew/lib", 2);
        Scan ("/usr/local/Cellar/python@3.8/3.8.11/Frameworks/Python.framework", 6);
      end if;
   end Search;
   ...
   begin
      Search (Major);
      if Most = null then
         return ""; -- Should be installed under the standard path
      else
         return Most.Path;
      end if;
   end Get_Python_Path;
