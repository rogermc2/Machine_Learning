...
# Sources:# [1] "The Digital Library of Mathematical Functions", dlmf.nist.gov
...
cdef inline double complex cdigamma(double complex z) nogil:
    ...
    if z.real < 0 and fabs(z.imag) < smallabsz:
        # Reflection formula for digamma. See https://dlmf.nist.gov/5.5#E4
    ...
    if zabs(z - posroot) < 0.5:
        res += zeta_series(z, post, posrootval)
    elif absz > smallabsz:
...
cdef inline double complex forward_recurrence(double complex z,
                                              double complex psiz,
                                               int n) nogil:
    """
    Compute digamma(z + n) using digamma(z) using the recurrence relation
...
cdef inline double complex backward_recurrence(double complex z, double complex psiz,
                                               int n) nogil:
    """
