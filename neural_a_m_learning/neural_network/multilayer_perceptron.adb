            ...
            while Continue and then iter < Self.Parameters.Max_Iter loop
                ...
                for batch_index in Batches.First_Index .. Batches.Last_Index loop
                    ...
                    for row in Batch_Slice.First .. Batch_Slice.Last loop
                        for col in Batch_Slice.First .. Batch_Slice.Last loop
                            X_Batch (row, col) := X (row, col);
                        end loop;
                        Y_Batch (row, 1) := Y (row, 1);
                    end loop;
        ...
        for layer in 1 .. Num_Layers - 1 loop
            ...
            begin
                Put_Line (Routine_Name & "Activations length:" &
                            Count_Type'Image (Activation'Length) & " x" &
                            Count_Type'Image (Activation'Length (2)));

            end;
            Put_Line (Routine_Name & "Coefficient_Matrix size:" &
                        Count_Type'Image (Self.Attributes.Params.Element (layer).Coeff_Params'Length) & " x" &
                        Count_Type'Image (Self.Attributes.Params.Element (layer).Coeff_Params'Length (2)));
            declare
                Coefficient_Matrix : constant Float_Matrix :=
                                       Self.Attributes.Params.Element (layer).Coeff_Params;
                Intercepts         : Float_Array :=
                                       Self.Attributes.Params.Element (layer).Intercept_Params;
                Acts_Dot_Coeffs    : constant Float_Matrix :=
                                       Dot (Activations (layer), Coefficient_Matrix);
                Act_With_Intercept : Float_Matrix := Coefficient_Matrix;
            begin
                --           Put_Line (Routine_Name & "layer:" & Integer'Image (layer));
