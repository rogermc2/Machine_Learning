...
package body Neural_Model is
   ...
   function Forward (aModel : in out Sequential_Model; Layer_ID : Positive;
                     Data   : Real_Float_Vector) return Real_Float_Vector;
   ...
   procedure Fit (aModel             : in out Sequential_Model;
                  Input_Data, Labels : Real_Float_Matrix;
                  Learning_Rate      : Float) is
      use Real_Float_Arrays;
      Routine_Name : constant String := "Neural_Model.Fit ";
      Output       : Real_Float_Vector (Input_Data'Range);
      --        Pred         : Real_Float_Matrix (Labels'Range, Labels'Range (2));
      --        Loss         : Real_Float_Matrix (Labels'Range, Labels'Range (2));
   ...
   begin
      ...
      for sample in 1 .. aModel.Num_Samples loop
         Output := Get_Row (Input_Data, sample);
         for layer in aModel.Layers.First_Index ..
           aModel.Layers.Last_Index loop
            Output := Forward (aModel, layer, Output);
         end loop;
         ...
         when Loss_Mean_Square_Error => null;
            --              Loss (sample, 1) := Base_Neural.Squared_Loss (Pred, );
   ...
   function Forward (aModel : in out Sequential_Model; Layer_ID : Positive;
                     Data   : Real_Float_Vector) return Real_Float_Vector is
      use Real_Float_Arrays;
      use Base_Neural;
      use Stochastic_Optimizers;
      use Layer_Packge;
      ...
      Connect      : constant Parameters_Record :=
                       aModel.Connections (Layer_ID - 1);
      Result       : Real_Float_Vector (Data'Range);
   begin

      aModel.Layers (Layer_ID).Input_Data := Data;
      declare
