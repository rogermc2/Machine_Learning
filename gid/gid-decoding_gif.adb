...
package body GID.Decoding_GIF is
   ...
   begin -- Load
      ...
      loop
         ...
         if full_trace then
            Ada.Text_IO.Put ("GIF separator [" & separator &
                               "][" & U8'Image (temp) & ']');
         end if;
         case separator is
         when ',' => -- 16#2C#
         ...
         when '!' => -- 16#21# Extensions
            ...
            when 16#F9# => -- See: 23. Graphic Control Extension
               ...
               image.next_frame :=
                 image.next_frame +
                   Ada.Calendar.Day_Duration (delay_frame) / 100.0;
               next_frame := image.next_frame;
               Get_Byte (image.buffer, temp);
            ...
            when others =>
               if full_trace then
                  Ada.Text_IO.Put_Line (" - Unused extension:" &
                                          U8'Image (label));
               end if;
               Skip_sub_blocks;
            end case;
         ...
         when ASCII.NUL =>
            ...
            if full_trace then
               Ada.Text_IO.Put_Line
                 (" - Wrong separator, skip and hope for the better...");
            end if;
         when others =>
            raise error_in_image_data with
      ...
      if new_num_of_colours < 256 then
         ...
         declare
            ...
            procedure GIF_Decode_general is
              new GIF_Decode (frame_interlaced, frame_transparency,
                              custom_pixel_mask);
         begin
            GIF_Decode_general;
         end;
