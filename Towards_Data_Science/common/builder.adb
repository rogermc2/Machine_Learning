...
package body Builder is
   ...
   begin
      Put_line ("Builder.Process_String_Value Feature_Name: " & "'" &
                  To_String (Feature) & "'"  & " value " &
                  To_String (Value));
      Question.Feature_Name := Feature;
      Question.UB_String_Value := Value;
      Split (Rows, Uncertainty, Question, Best);
      Utilities.Print_Question ("Builder.Process_String_Value", Question);
      Utilities.Print_Best ("Builder.Process_String_Value", Best);
      New_Line;
   ...
   begin
      ...
      if not Split_Row.True_Rows.Is_Empty and then
        not Split_Row.False_Rows.Is_Empty then
         ...
         Put_Line ("Split Gain: " & Float'Image (Question.Gain));
