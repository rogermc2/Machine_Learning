...
package body Builder is
    ...
    function Build_Tree (Rows : Rows_Vector) return Tree_Type is
        ...
        begin
            ...
            if Best_Split.Gain = 0.0 then
                ...
                --                  Print_Rows ("Builder.Build_Tree.Recurse Node.True_Rows", Node.True_Rows);
                --                  Print_Rows ("Builder.Build_Tree.Recurse Node.False_Rows", Node.False_Rows);
                theTree.Insert_Child (Parent   => Curs,
                                      Before   => No_Element,
                                      New_Item => Node,
                                      Position => Node_Curs);
    ...
    end Process_Boolean_Value;

    --  -----------------------------------------------------------------------

    procedure Process_Float_Value
    ...
    begin
        ...
        Split (Rows, Uncertainty, Question, Best);
    end Process_Float_Value;

    --  -----------------------------------------------------------------------

    procedure Process_Integer_Value
    ...
    begin
        ...
        Split (Rows, Uncertainty, Question, Best);
    end Process_Integer_Value;

    --  -----------------------------------------------------------------------

    procedure Process_String_Value
...
--          Put_Line ("Builder.Process_String_Values, Question data Value: " &
--                      To_String (Value));
--          New_Line;
        Split (Rows, Uncertainty, Question, Best);
    end Process_String_Value;
            ...
            for col in 1 .. Num_Features loop
                ...
                for row in
                  Positive'Succ (Rows.First_Index) .. Rows.Last_Index loop
                    ...
                    when Boolean_Type =>
                        Put_Line ("Builder.Find_Best_Split for Boolean_Type");
    ...
    begin
        --        New_Line;
        --        Put_Line ("Builder.Split question data, Feature " &
        --                    To_String (Question.Feature_Name));
