...
package body Utilities is
    ...
    package Values_Package is new Ada.Containers.Ordered_Maps
      (Class_Range, Value_Data);
    subtype Value_Set is Values_Package.Map;
    ...
    procedure Print_Unique_Values (Rows    : Rows_Vector;
        ...
        Data   : Value_Data;
    begin
        Put ("Unique " & To_String (Feature) & " Values:");
            ...
            for index in Class_Range range
              Class_Range'Succ (Class_Range (Rows.First_Index)) ..
                ...
                for col in Class_Range range
                  1 .. Class_Range (Rows.Length) - 1 loop
                    Feature_Name :=
                      Feature_Name_Type (Rows.First_Element.Features (col));
                    if Feature_Name = Feature then
                        Feature_Data_Type :=
                          Get_Data_Type (Row2_Features (index));
                        Value_String := Data.Features (index);
