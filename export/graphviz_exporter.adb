...
package body Graphviz_Exporter is
    ...
    procedure Recurse (Exporter : in out DOT_Tree_Exporter;
        ...
        procedure Do_Node (Curs : Cursor) is
            Left_Child   : Cursor;
            Right_Child  : Cursor;
            Node_ID      : constant String := Integer'Image (Element (Curs).Node_ID);
            Depth_S      : constant String := Integer'Image (Depth);
            Depth_Pos    : constant Natural := Index (Exporter.Ranks, Depth_S);
            Depth_Length : constant Natural := Depth_S'Length;
        begin
            if Curs /= Exporter.theTree.Root and then
              ...
              Depth <= Exporter.Max_Depth then
                ...
                if Element (Left_Child).Leaf_Node then
                    Exporter.Ranks := To_Unbounded_String ("leaves" & Node_ID);
                elsif Depth_Pos = 0 then
                    Exporter.Ranks := To_Unbounded_String (Depth_S);
                    Replace_Slice (Exporter.Ranks, 1, Depth_Length, Node_ID);
                else
                    Exporter.Ranks := To_Unbounded_String (Depth_S & Node_ID);
                end if;
