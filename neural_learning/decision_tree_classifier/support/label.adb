...
package body Label is
   ...
   function Label_Binarize2 (Y         : Unbounded_String_Array;
     ...
     subtype Y_UB_String is Unbounded_String_Array (1 .. Y'Length);
      Classes_Array : constant Unbounded_String_Array := To_Unbound_Array (Classes);
      package Label_Binarize_UB is new Generic_Label_Binarize
     (Index_Type => Integer, Class_Type => Unbounded_String,
      Y_Array_Type  => Y_UB_String ,
      Class_Array_Type => Unbounded_String_Array (Classes_Array'Range),
      Type_Of_Target   => Multiclass_Utils.Type_Of_Target,
      "<" => Ada.Strings.Unbounded."<");
   begin
      return Label_Binarize_UB (Y, Classes_Array);
   end Label_Binarize2;
