...
begin
   ...
   for index in First_Moments.First_Index .. First_Moments.Last_Index loop
      First_Moments (index).Coeff_Grads :=
        Beta_1 (1) * First_Moments (index).Coeff_Grads +
        (1.0 - Beta_1 (1)) * Grads (index).Coeff_Grads;
      First_Moments (index).Intercept_Grads :=
        Beta_1 (1) * First_Moments (index).Intercept_Grads +
        (1.0 - Beta_1 (1)) * Grads (index).Intercept_Grads;
      declare
         Grad_Sq : constant Parameters_Record := Square (Grads (index));
      begin
         Second_Moments (index).Coeff_Grads :=
           Beta_2 (1) * Second_Moments (index).Coeff_Grads +
           (1.0 - Beta_2 (1)) * Grad_Sq.Coeff_Grads;
         Second_Moments (index).Intercept_Grads :=
           Beta_2 (1) * Second_Moments (index).Intercept_Grads +
           (1.0 - Beta_2 (1)) * Grad_Sq.Intercept_Grads;
      end;
   end loop;
