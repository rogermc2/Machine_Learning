...
package body Samples_Generator is
   ...
   begin
      ...
      Test_Support.Print_Float_Matrix (Routine_Name & "Centroids", Centroids);
      Test_Support.Print_Integer_Array (Routine_Name & "N_Samples_Per_Cluster",
                                         N_Samples_Per_Cluster);
      for row in Centroids'Range loop
         K := K + 1;
         Put_Line (Routine_Name & "K, row:" & Integer'Image (K) &
                     Integer'Image (row));
         for col in Centroids'Range (2) loop
            Start := Stop + 1;
            Stop := Stop + N_Samples_Per_Cluster (K);
            Put_Line (Routine_Name & "Stop:" & Integer'Image (Stop));
            for y_index in Start .. Stop loop
               Put_Line (Routine_Name & "y_index:" & Integer'Image (y_index));
               --  assign label
               Y (y_index) := K mod N_Classes;
   ...
   begin
      ...
      Test_Support.Print_Binary_Matrix (Routine_Name & "Hypercube",
                                        Hypercube);
