...
--        Deltas_Curs  : Cursor := Deltas.First;
   begin
      Put_Line (Routine_Name & "Deltas (Index - 1) size:" &
                  Count_Type'Image (Deltas.Element (Index - 1)'Length) & " x" &
                  Count_Type'Image (Deltas.Element (Index - 1)'Length (2)));
      ...
      Put_Line (Routine_Name & "index, Deltas length:" &
                  Integer'Image (index) & ", " &
                  Count_Type'Image (Deltas.Length));
      --  L311
      Deltas.Prepend (Deltas.Element (Index - 1) *
                        Transpose (Params.Coeff_Grads));
      Put_Line (Routine_Name & "L311+");
      Put_Line (Routine_Name & "updated Deltas length:" &
                  Count_Type'Image (Deltas.Length));
--        Deltas_Curs := Deltas.First;
      Put_Line (Routine_Name & "updated Deltas (Index - 1) size:" &
                  Count_Type'Image (Deltas.Element (Index - 1)'Length) & " x" &
                  Count_Type'Image (Deltas.Element (Index - 1)'Length (2)));
      ...
      case Self.Parameters.Activation is
         ...
         when Rect_LU_Activation =>
            Rect_LU_Derivative (Activations (index), Deltas (Index - 1));
