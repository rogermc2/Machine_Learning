...
package body Base_Neural is
   ...
   function Log_Loss (Y_True, Y_Prob : Real_Float_Matrix) return Float is
      ...
      YT2          : Real_Float_Matrix := Y_True;
      YP2          : Real_Float_Matrix (Y_Prob'Range, Y_Prob'Range (2));
   ...
   begin
      --  L194 Clip Y_Prob to range EPS .. 1.0 - EPS
      Clip (YP_Clip);
      for row in YP_Clip'Range loop
         for col in YP_Clip'Range (2) loop
            YT2 (row, col) := 1.0 - YT2 (row, col);
            YP2 (row, col) := 1.0 - Y_Prob (row, col);
         end loop;
      end loop;
      Clip (YP2);

      return - (Sum_XlogY (Y_True, YP_Clip) + Sum_XlogY (YT2, YP2)) /
        Float (YP_Clip'Length);
   ...
   function Log_Loss (Y_True : Integer_Matrix; Y_Prob : Real_Float_Matrix)
      ...
      YP2          : Real_Float_Matrix
        (YP'Range, YP'First (2) .. YP'Last (2) + 1);
