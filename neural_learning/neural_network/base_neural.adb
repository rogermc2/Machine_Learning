...
package body Base_Neural is
   ...
   function X_Log_Y (X : Binary_Matrix; Y : Real_Float_Matrix)
                     return Real_Float_Matrix;
   ...
   begin
      for row in X'Range loop
         for col in X'Range (2) loop
            for y_col in Y'Range (2) loop
               if X (row, col) = 1 then
                  if Y1 (row, y_col) < EPS then
                     Y1 (row, y_col) := EPS;
                  end if;
                  if X (row, col) = 1 then
                     Result (row, y_col) :=  Log (Y1 (row, y_col));
                  end if;
               end if;
            end loop;
         end loop;
