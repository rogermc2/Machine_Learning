...
--                              return Float_List;
   ...
   function Get_Colour (Exporter : in out DOT_Tree_Exporter;
      ...
      function Max_Bound (Bounds : Export_Types.Graph_Bounds) return Float is
         Max_Val : Float := Bounds.H;
      begin
         if abs (Bounds.V) > Max_Val then
            Max_Val := Bounds.V;
         end if;
         return Max_Val;
      end Max_Bound;
   ...
   begin
      Assert (not Value.Is_Empty, Routine_Name & "Value Is_Empty");
              ...
              Child_Count (Node_Curs) > 0 then
               ...
               if Element (Left_Child).Leaf_Node then
                  ...
                  Include (Exporter.Ranks, To_Unbounded_String ("leaves"),
                           Node_ID_UB);
               elsif Exporter.Ranks.Contains (Depth_S) then
            ...
            if Node_ID > 1 then
               ...
               if Parent_ID = 1 then
                  ...
                  for index in Angles'First .. Angles'Last loop
                     Angles (index) :=
                       - Integer ((2.0 * Exporter.Rotate - 1.0) *
                                    Base_Angles (index));
                  end loop;
                  Put (Output_File,
