...
package body Utilities is
    ...
    function Permute (aList : ML_Types.Value_Data_Lists_2D)
        ...
        type Random_Integer_Range is range 1 .. List_Length;
        Rand         : Positive;
    ...
    begin
        if List_Length > 1 then
            for index in 1 .. List_Length - 1 loop
                declare
                    Index_Val : constant Positive := index;
                ...
                begin
                    Rand := Random (Integer_Gen);
                    Put_Line ("Utilities.Permute index, Rand" &
                                Integer'Image (index) & ", " & Integer'Image (Rand));
