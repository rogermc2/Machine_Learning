...
--                    Put_Line ("Builder.Find_Best_Split UB_String_Type.");
   ...
   begin
      ...
      if Found then
         ...
         case Val_Type is
            when Integer_Type =>
               Utilities.Print_Question ("Builder.Match Integer_Type ", Question);
               declare
                  Question_Value : constant Integer := Question.Integer_Value;
               begin
                  Put_Line
                    ("Builder.Match Integer_Type Question_Value, Example_Feature: "
                     & Integer'image (Question_Value) & "  " &
                       To_String (Example_Feature));
   ...
   begin
      Put_Line ("Builder.Split partition rows.");
      Split_Rows := Partition (Rows, Question);
      Put_Line ("Builder.Split rows partitioned.");
