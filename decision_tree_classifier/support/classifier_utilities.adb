...
package body Classifier_Utilities is
   ...
   --                  Max_Indices.Append (Max (Values.Element (index_2)));
   --                  Samples_2K.Clear;
   --                  for index_3 in Class_Values.First_Index .. Class_Values.Last_Index loop
   ...
   function Parse_Row_Data (Row_Data : ML_Types.Raw_Data_Vector)
      ...
      aRow           : Unbounded_String := Row_Data.First_Element;
      Header_Data    : constant Header_Data_Type
        := Builder.Parse_Header (To_String (aRow));
      Num_Features   : constant Class_Range
        := Class_Range (Ada.Strings.Fixed.Count (To_String (aRow), ","));
      Values         : Feature_Data_Array (1 .. Num_Features);
      CSV_Line       : String_List;
      Curs           : String_Package.Cursor;
      Value_Index    : Class_Range;
      Row            : ML_Types.Row_Data (Num_Features);
      ...
      Data           : Data_Record;
   begin
      Feature_Types := Header_Data.Feature_Types;
      for index in Header_Data.Features'First .. Header_Data.Features'Last loop
         Data.Feature_Names.Append (Header_Data.Features (index));
      end  loop;

      for row_index in Positive'Succ (Row_Data.First_Index) .. Row_Data.Last_Index loop
         Value_Index  := 1;
         aRow := Row_Data (row_index);
         ...
         Row.Features := Values;
         Data.Feature_Values.Append (Row);
      end loop;
