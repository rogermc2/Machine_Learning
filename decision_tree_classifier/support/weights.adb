...
package body Weights is
    ...
    begin
        ...
        for k in Classes.First_Index .. Classes.Last_Index loop
            --  Compute_Class_Weights input parameter is a Weight_List
            Class_Weight_List.Clear;
            Y_Full := Classifier_Utilities.Get_Column (Y, k);
            Classes_Full := Encode_Utils.Unique (Y_Full, Inverse);
            Classes_Missing.Clear;
            Class_Weight := Class_Weights.Element (k);
            Class_Weight_List.Append (Class_Weight);
            Classifier_Utilities.Print_Weights
              ("Compute_Balanced_Sample_Weight Class_Weight_List", Class_Weight_List);
            ...
            Weight_K := Compute_Class_Weights
              (Balanced_Weight, Class_Weight_List, Classes_Full, Y_Full);
            ...
            for index in Class_Weight_List.First_Index .. Class_Weight_List.Last_Index loop
                Expanded_Class_Weight.Append (Class_Weight_List.Element (index));
            end loop;
        end loop;
