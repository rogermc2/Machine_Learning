...
package body Node_Splitter is
    ...
    begin
        ...
        if Best.Pos_I <= Self.Start_Index then
            raise Node_Splitter_Error with
              "Node_Splitter.Find_Best_Split, final position" &
              Integer'Image (Best.Pos_I) &
              " should be greater than Start_Index" &
              Integer'Image (Self.Start_Index);
        end if;
    ...
    begin
        ...
        Classifier_Utilities.Print_Natural_List
          ("Node_Splitter Reorganize_Samples, reorganised X_Samples",
           X_Samples);
    ...
    begin
        ...
        Reorganize_Samples (Self, Best_Split, Self.Sample_Indices, Impurity);
        if Best_Split.Pos_I <= Self.Start_Index then
            raise Node_Splitter_Error with
