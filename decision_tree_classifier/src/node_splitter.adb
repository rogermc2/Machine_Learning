...
package body Node_Splitter is
   ...
   begin
      ...
      Put_Line ("Node_Splitter.Find_Best_Split start, current position" &
           Integer'Image (Current.Split_Row) &
           " Start_Row" & Integer'Image (Self.Start_Row));
      --  L379 Evaluate all splits
      Best := Current;
      Criterion.Reset (Self.Criteria);
      Put_Line ("Node_Splitter.Find_Best_Split L379 reset");

      --  Set of features to be split :
      --  P_Index: Self.Start_Index through Self.End_Index
      while P_Index <= Self.End_Row loop
         Put_Line ("Node_Splitter.Find_Best_Split outer P_Index: " &
                  Integer'Image (P_Index));
         LE := True;
         while P_Index + 1 < Self.End_Row and LE loop
            Put_Line ("Node_Splitter.Find_Best_Split inner P_Index: " &
                        Integer'Image (P_Index));
            Classifier_Utilities.Print_Value_Data_List
              ("Find_Best_Split Features_X", Features_X);
   ...
   begin
      ...
      --  L323
      Put_Line ("Node_Splitter.Split_Node L323");
      while F_I > Num_Total_Constants and
        (Num_Visited_Features < Max_Features or
         ...
         if F_J < Num_Known_Constants then
            ...
            Put_Line ("Node_Splitter.Split_Node L356");
            --  L356
