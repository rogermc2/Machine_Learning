...
--                              Integer'Image (F_J));
                ...
                --  L358 Sort samples along Current.Feature;
                Current.Feature := Features.Element (F_J);
                Put_Line ("Node_Splitter.Process_Constants curretn Feature index: " &
                ...
                for index in Self.Start_Index .. Self.End_Index loop
                    ...
                    X_Sample_I := Self.Sample_Indices.Element (Samples_I);
                    --  X_Features is a Value_Data_List
                    X_Features := Self.X.Element (X_Sample_I);
                    Classifier_Utilities.Print_Value_List
                      ("Node_Splitter.Process_Constants X_Features",
                       X_Features);
                    Put_Line ("Node_Splitter.Process_Constants, Feature_Values length: "
                              & Integer'Image (Integer (Self.Feature_Values.Length)));
                    Put_Line ("Node_Splitter.Process_Constants, Current.Feature: "
                              & Integer'Image (Current.Feature));
                    Self.Feature_Values.Replace_Element
                      (Features (index), X_Features (Current.Feature));
                    Put_Line
                      ("Node_Splitter.Process_Constants Features (index) replaced");
                end loop;

                Put_Line ("Node_Splitter.Process_Constants L367");
                --  L367
