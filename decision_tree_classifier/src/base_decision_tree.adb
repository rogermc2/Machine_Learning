...
package body Base_Decision_Tree is
    ...
    function Predict (Self : in out Classifier;
        ...
        Max_Args         : Natural_List;
        --  Predictions, num samples x num outputs
        Predictions      : Weight_Lists_2D;
    begin
        ...
        for k in 1 .. Positive (Self.Attributes.Num_Outputs) loop
            ...
            Max_Args := Max (Prob_A_K, 2);
            Selected_Classes.Clear;
            for args_index in Max_Args.First_Index .. Max_Args.Last_Index loop
                Class_K := Self.Attributes.Classes.Element (Max_Args.Element (args_index));
