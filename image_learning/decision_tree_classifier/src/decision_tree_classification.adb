...
package body Decision_Tree_Classification is
   ...
   function Predict_Probability (Self : in out Base_Decision_Tree.Classifier;
      ...
      Proba       : constant Weight_Lists_2D :=
   ...
   begin
      ...
      for class_index in Self.Attributes.Classes.First_Index
        .. Self.Attributes.Classes.Last_Index loop
         Normalizer := 0.0;
         --              F_Class := Node_K.Element (class_index);
         F_Class := Self.Attributes.Classes.Element (class_index);
         --              for class_index in Node_K.First_Index ..
         --                Node_K.Last_Index loop
         --                  F_Class := Node_K.Element (class_index);
         --                  Normalizer := Normalizer + F_Class;
         --              end loop;
         Normalizer := Normalizer + Float (F_Class);
         ...
         if Normalizer > 0.0 then
            ...
            for class_index in Self.Attributes.Classes.First_Index
              .. Self.Attributes.Classes.Last_Index loop
               ...
               F_Class := Self.Attributes.Classes (class_index);
               Self.Attributes.Classes.Replace_Element
                 (class_index, Integer (Float (F_Class) / Normalizer));
            end loop;
         end if;
      end loop;
...
--        Node_K      : Weight_List;
      ...
      Proba       : constant Weight_Lists_2D := Predict_Probability (Self, X);
      Log_Class   : Float;
   begin
      ...
      for class_index in Self.Attributes.Classes.First_Index
        .. Self.Attributes.Classes.Last_Index loop
         Log_Class :=
           Log (Float (Self.Attributes.Classes.Element (class_index)));
         Self.Attributes.Classes.Replace_Element (class_index, Integer (Log_Class));
