...
package body Openml_Ada is
    ...
    procedure Get_OML (File_Name : String;
                       X, Y      : out IL_Types.Value_Data_Lists_2D;
    ...
    procedure Save_OML
      (Save_File_Name        : String; X, Y : IL_Types.Value_Data_Lists_2D;
    ...
    procedure Fetch_Openml
      (Dataset_File_Name : String;
       ...
       X                 : out Classifier_Types.Value_Data_Lists_2D;
       Y                 : out Classifier_Types.Value_Data_Lists_2D;
       X_Indices         : out IL_Types.Integer_List;
       Y_Indices         : out IL_Types.Integer_List;
       Bunch             : out Bunch_Data;
    ...
    begin
        if Exists (Save_File_Name) then
            Get_OML (Save_File_Name, X, Y, X_Indices, Y_Indices, Bunch,
                     Return_X_Y);
        else
            ...
            if Save_File_Name'Length > 0 then
                Save_OML (Save_File_Name, X, Y, X_Indices, Y_Indices, Bunch,
                          Return_X_Y);
            end if;
    ...
    begin
        ...
        Value_Data_Lists_2D'Read (aStream, X);
        Value_Data_Lists_2D'Read (aStream, Y);
    ...
    begin
        ...
        aStream := Stream (File_ID);
        Value_Data_Lists_2D'Write (aStream, X);
        Value_Data_Lists_2D'Write (aStream, Y);
    ...
    function Split_Columns
      ...
      return Classifier_Types.Float_List_2D is
        ...
        Arff_Data_Row : Float_List;  --  list of columns
        New_Row       : Float_List;
    begin
    ...
    begin
        for row in Arff_Target.First_Index .. Arff_Target.Last_Index loop
            New_Row.Clear;
            Arff_Value := Arff_Target.Element (row);
            Include_Curs := Include_Columns.First;
            while Has_Element (Include_Curs) loop
                declare
                    Arff_Value : constant Integer :=
                                   Arff_Target.Element (Element (Include_Curs));
                begin
                    Data_New.Append (Arff_Value);
                end;
                Next  (Include_Curs);
            end loop;
