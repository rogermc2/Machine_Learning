...
--     background_image_name : Unbounded_String := Null_Unbounded_String;
   ...
   begin
      ...
      Put_Line (Standard_Error, "  Image format: " &
                  GID.Image_format_type'Image (GID.Format (i)));
      Put_Line (Standard_Error, "  Image detailed format: " &
                  GID.Detailed_format (i));
      Put_Line (Standard_Error, "  Image sub-format ID (if any): " &
                  Integer'Image (GID.Subformat (i)));
      Put_Line (Standard_Error, "  Dimensions in pixels: " &
      ...
      Put (Standard_Error, "  Color depth: " &
      ...
      Put_Line (Standard_Error, "  Palette: " &
                  Boolean'Image (GID.Has_palette (i)));
      Put_Line (Standard_Error,
                "  Greyscale: " & Boolean'Image (GID.Greyscale (i)));
      Put_Line (Standard_Error, "  RLE encoding (if any): " &
                  Boolean'Image (GID.Is_RLE_encoded (i)));
      Put_Line (Standard_Error, "  Interlaced (GIF: each frame's choice): " &
                  Boolean'Image (GID.Is_Interlaced (i)));
      Put_Line (Standard_Error, "  Expect transparency: " &
      ...
      if as_background then
         case GID.Display_orientation (i) is
         when GID.Unchanged =>
            Load_raw_image_0 (i, bkg_buf, next_frame, background_image_name);
         when GID.Rotation_90 =>
            Load_raw_image_90 (i, bkg_buf, next_frame, background_image_name);
         when GID.Rotation_180 =>
            Load_raw_image_180 (i, bkg_buf, next_frame, background_image_name);
         when GID.Rotation_270 =>
            Load_raw_image_270 (i, bkg_buf, next_frame, background_image_name);
         end case;
      ...
      loop
         case GID.Display_orientation (i) is
         when GID.Unchanged =>
            Load_raw_image_0 (i, img_buf, next_frame, background_image_name);
         when GID.Rotation_90 =>
            Load_raw_image_90 (i, img_buf, next_frame, background_image_name);
         when GID.Rotation_180 =>
            Load_raw_image_180 (i, img_buf, next_frame, background_image_name);
         when GID.Rotation_270 =>
            Load_raw_image_270 (i, img_buf, next_frame, background_image_name);
         end case;

         if not test_only then
            Dump_BMP_24 (name & '_' &
                           Trim (Duration'Image (current_frame), Left), i);
         end if;
