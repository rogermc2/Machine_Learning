...
package body Mapping is
   ...
   --  ------------------------------
   procedure Set_Member (Person : in out Person_Data;
   ...
   begin
      ...
      case Field is
         when FIELD_FIRST_NAME =>
            Person.First_Name := To_Unbounded_String (Value);

         when FIELD_LAST_NAME =>
            Person.Last_Name := To_Unbounded_String (Value);

         when FIELD_AGE =>
            Person.Age := To_Integer (Value);

         when FIELD_ID =>
            Person.Id := To_Long_Long_Integer (Value);

         when FIELD_NAME =>
            Person.Name := To_Unbounded_String (Value);

         when FIELD_USER_NAME =>
            Person.Username := To_Unbounded_String (Value);

         when FIELD_GENDER =>
            Person.Gender := To_Unbounded_String (Value);

         when FIELD_LINK =>
            Person.Link := To_Unbounded_String (Value);
   ...
   --  Set the name/value pair on the current object.
   --  ------------------------------
   function Get_Person_Member (From  : in Person_Data;
