...
package body Load_ARFF_Data is
   ...
   procedure Decode_Nominal (Attribute      : Attribute_Record;
                             Value_String   : String;
                             Decoded_Values : in out ML_Types.ARFF_Data_List);
   ...
   function Decode_Dense_Values (Values     : ML_Types.Indef_String_List;
                                 Attributes : Attribute_List)
                                  return ML_Types.ARFF_Data_List is
      use Ada.Containers;
      use Ada.Strings;
      use ML_Types;
      ...
      while Has_Element (Attr_Cursor) loop
         ...
         begin
            case ARFF_Data_Kind is
               when ARFF_Integer =>
                  ...
                  begin
                     if Fixed.Index (Value_String, ".") /= 0 then
                        Value.Integer_Value := Integer (Float'Value (Value_String));
                     else
               ...
               when ARFF_Nominal =>
                  Decode_Nominal (Attribute, Value_String, Decoded_Values);
   ...
   procedure Decode_Nominal
     (Attribute       : Attribute_Record; Value_String : String;
      Decoded_Values  : in out ML_Types.ARFF_Data_List) is
      use ML_Types;
                  ...
                  case Nominal.Data_Kind is
                     when Nominal_String =>
                        declare
                           Value : Value_Record (UB_String_Type);
                        begin
                           Value.UB_String_Value := Nominal.UB_String_Data;
                           Decoded_Values.Append (Value);
                        end;
                     when Nominal_Integer =>
                        declare
                           Value : Value_Record (Integer_Type);
                        begin
                           Value.Integer_Value := Nominal.Integer_Data;
                           Decoded_Values.Append (Value);
                        end;
                     when Nominal_Real | Nominal_Numeric =>
                        declare
                           Value : Value_Record (Float_Type);
                        begin
                           Value.Float_Value := Nominal.Real_Data;
                           Decoded_Values.Append (Value);
                        end;
                  end case;
               end if;
